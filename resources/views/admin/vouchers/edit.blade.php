<x-admin-layout>
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-black bg-gradient-to-r from-emerald-400 to-teal-500 bg-clip-text text-transparent">Edit Voucher</h1>
            <p class="text-slate-400 text-sm mt-1">Ubah ketentuan atau masa berlaku voucher {{ $voucher->kode_voucher }}.</p>
        </div>
            ...
            <form action="{{ route('admin.vouchers.update', $voucher->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="kode_voucher" id="kode_voucher" value="{{ old('kode_voucher', $voucher->kode_voucher) }}" required
                    ...
                    <div>
                        ...
                        <input type="number" name="diskon" id="diskon" value="{{ old('diskon', (int) $voucher->diskon) }}" required min="0"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="tanggal_mulai" id="tanggal_mulai" value="{{ old('tanggal_mulai', $voucher->tanggal_mulai->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_berakhir" id="tanggal_berakhir" value="{{ old('tanggal_berakhir', $voucher->tanggal_berakhir->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-slate-200 focus:outline-none focus:border-emerald-500 transition">
                        ...
                        <option value="tidak_aktif" {{ old('status', $voucher->status) === 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
                    @error('status')
                        <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
